...
import com.nju.edu.erp.dao.Initial_ProductDao;
...
public class ProductServiceImpl implements ProductService {
    ...
    @Autowired
    private Initial_ProductDao initial_productDao;
    ...
    @Override
    @Transactional
    public void initial_createProduct(CreateProductVO inputVO) {
        // 查找商品对应的分类
        CategoryPO category = categoryDao.findByCategoryId(inputVO.getCategoryId());
        // 生成商品ID
        String productId = generateProductId(category);
        // 增加商品信息
        Initial_ProductPO savePO = new Initial_ProductPO();
        BeanUtils.copyProperties(inputVO, savePO);
        savePO.setId(productId);
        initial_productDao.createProduct(savePO);
        // 同步修改分类下的商品数量和商品index
        category.setItemCount(category.getItemCount() + 1);
        category.setItemIndex(category.getItemIndex() + 1);
        categoryDao.updateById(category);
    ...
    public void initialcreate() {
        initial_productDao.initialcreate();
    }
    ...
    @Override
    @Transactional
    public List<ProductInfoVO> queryAllInitialProduct() {
        List<Initial_ProductPO> queryAns = initial_productDao.selectList(null);
        List<ProductInfoVO> responseVO = queryAns.stream().map(productPO -> {
            ProductInfoVO productInfoVO = new ProductInfoVO();
            BeanUtils.copyProperties(productPO, productInfoVO);
            return  productInfoVO;
