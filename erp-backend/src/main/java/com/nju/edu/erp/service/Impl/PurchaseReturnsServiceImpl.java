...
public class PurchaseReturnsServiceImpl implements PurchaseReturnsService {
    ...
    @Autowired
    public PurchaseReturnsServiceImpl(PurchaseReturnsSheetDao purchaseReturnsSheetDao, ProductService productService, CustomerService customerService, WarehouseService warehouseService, PurchaseSheetDao purchaseSheetDao, WarehouseDao warehouseDao) {
    ...
    @Override
    @Transactional
    public void approval(String purchaseReturnsSheetId, PurchaseReturnsSheetState state) { // TODO 这代码也写的太烂了吧
        ...
        if (state.equals(PurchaseReturnsSheetState.FAILURE)) {
            ...
            PurchaseReturnsSheetState prevState;//前一个状态
    ...
    @Override
    @Transactional
    public List<PurchaseReturnsSheetVO> getPurchaseReturnsSheetByTime(String beginDateStr, String endDateStr) {
        ...
        try {
            ...
            if (beginTime.compareTo(endTime) > 0) {
                ...
                all = purchaseReturnsSheetDao.findAllSheetByTime(beginTime, endTime);
                for (PurchaseReturnsSheetPO po : all) {
                    PurchaseReturnsSheetVO vo = new PurchaseReturnsSheetVO();
                    BeanUtils.copyProperties(po, vo);
                    List<PurchaseReturnsSheetContentPO> allContent = purchaseReturnsSheetDao.findContentBySheetId(po.getId());
                    List<PurchaseReturnsSheetContentVO> vos = new ArrayList<>();
                    for (PurchaseReturnsSheetContentPO p : allContent) {
                        PurchaseReturnsSheetContentVO v = new PurchaseReturnsSheetContentVO();
                        BeanUtils.copyProperties(p, v);
                        vos.add(v);
                    }
