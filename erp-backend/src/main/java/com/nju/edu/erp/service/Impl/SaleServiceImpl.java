...
public class SaleServiceImpl implements SaleService {
    ...
    public void makeSaleSheet(UserVO userVO, SaleSheetVO saleSheetVO) {
        ...
        //折后总价
        saleSheetPO.setFinalAmount(rawTotalAmount.multiply(saleSheetPO.getDiscount()).subtract(saleSheetPO.getVoucherAmount()));
    ...
    public List<SaleSheetVO> getSaleSheetByState(SaleSheetState state) {
        ...
        for (SaleSheetPO po : all) {
            ...
            List<SaleSheetContentPO> allContent = saleSheetDao.findContentBySheetId(po.getId());
            List<SaleSheetContentVO> vos = new ArrayList<>();
            for (SaleSheetContentPO p : allContent) {
    ...
    @Override
    @Transactional
    public List<SaleSheetVO> getSaleSheetByTime(String beginDateStr, String endDateStr) {
        ...
        try {
            ...
            if (beginTime.compareTo(endTime) > 0) {
                ...
                all = saleSheetDao.findAllSheetByTime(beginTime, endTime);
                for (SaleSheetPO po : all) {
                    SaleSheetVO vo = new SaleSheetVO();
                    BeanUtils.copyProperties(po, vo);
                    List<SaleSheetContentPO> allContent = saleSheetDao.findContentBySheetId(po.getId());
