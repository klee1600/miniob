...
import com.nju.edu.erp.model.entity.WarehouseView;
...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public List<WarehouseIODetailPO> getWarehouseIODetailByTime(String beginDateStr, String endDateStr) {
        // TODO 已完成
        Date beginDate = str2Date(beginDateStr);
        Date endDate = str2Date(endDateStr);
        if (endDate.compareTo(beginDate) < 0)
            return null;
        List<WarehouseIODetailPO> res = new ArrayList<>();
        List<WarehouseIODetailPO> inputRes = warehouseInputSheetDao.getWarehouseInputDetailByTime(beginDate, endDate);
        for (WarehouseIODetailPO po : inputRes) {
            po.setType("warehouse_input");
        }

        List<WarehouseIODetailPO> outputRes = warehouseOutputSheetDao.getWarehouseOutputDetailByTime(beginDate, endDate);
        for (WarehouseIODetailPO po : outputRes) {
            po.setType("warehouse_output");
        }
    ...
    public int getWarehouseInputProductQuantityByTime(String beginDateStr, String endDateStr) {
        ...
        Date beginDate = str2Date(beginDateStr);
        Date endDate = str2Date(endDateStr);
        Integer i = warehouseInputSheetDao.getWarehouseInputProductQuantityByTime(beginDate, endDate);
        return i == null ? 0 : i;
    }
    ...
    public int getWarehouseOutProductQuantityByTime(String beginDateStr, String endDateStr) {
        ...
        Date beginDate = str2Date(beginDateStr);
        Date endDate = str2Date(endDateStr);
        Integer i = warehouseOutputSheetDao.getWarehouseOutputProductQuantityByTime(beginDate, endDate);
        return i == null ? 0 : i;
    ...
    private Date str2Date(String str) {
        ...
        try {
            ...
            throw new MyServiceException("500", "时间转换异常！");
        }
        return date;
    }
    ...
    public void download(HttpServletResponse response) {
        ...
        try {
            ...
            // 设置文件名
            response.addHeader("Content-Disposition", "attachment;fileName=" + URLEncoder.encode("库存盘点.xls", "UTF-8"));
    ...
    @Override
    public WarehousePO findByPidDesc(String pid) {
        return warehouseDao.findByPidDesc(pid);
    }

    @Override
    public void addQuantity(WarehousePO warehousePO, Integer quantity) {
        warehousePO.setQuantity(warehousePO.getQuantity() + quantity);
        warehouseDao.updateById(warehousePO);
    }

    private Workbook getWorkbook() {
        List<WarehouseCountingVO> arr = warehouseCounting();
        List<WarehouseView> view = new ArrayList<>();
        for (WarehouseCountingVO vo : arr) {
            view.add(WarehouseView.VO2View(vo));
        }
        ExportParams params = new ExportParams("库存盘点", "库存盘点", ExcelType.HSSF);
        return ExcelExportUtil.exportExcel(params, WarehouseView.class, view);
    }
}
