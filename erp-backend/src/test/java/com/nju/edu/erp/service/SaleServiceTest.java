...
public class SaleServiceTest { // 该测试为集成测试，需要用到数据库，请大家连给定的测试数据库进行测试
    ...
    @Transactional
    @Rollback(value = true)
    public void getMaxAmountCustomerOfSalesmanByTimeTest() {
        String salesman = "xiaoshoujingli";
        String beginDateStr = "2022-05-23 00:00:00";
        String endDateStr = "2022-05-24 02:00:00";

        CustomerPurchaseAmountPO actual = saleService.getMaxAmountCustomerOfSalesmanByTime(salesman, beginDateStr, endDateStr);
        CustomerPO customer = actual.getCustomerPO();
        CustomerPO expect = new CustomerPO(2, "销售商", 1, "lxs", "12580", "南哪儿大学", "123457", "12121@cba.com", BigDecimal.valueOf(20000000.00), BigDecimal.valueOf(4959100.00), BigDecimal.valueOf(0.00), "uncln");

        Assertions.assertEquals(8608700.00, actual.getTotalFinalAmount().doubleValue());
        Assertions.assertEquals(expect, customer);
    }
