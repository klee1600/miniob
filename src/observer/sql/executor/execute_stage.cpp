...
void print_aggregation_header(std::ostream &os, SelectStmt *select_stmt) {
  ...
  auto aggr_op_index = select_stmt->arggrops_idx_in_fields();
  auto sum = aggregation_ops.size() + query_field.size();
  int querycnt = 0, aggropscnt = 0;
  for (int i = 0; i < sum; ++i) {
    while (i < aggr_op_index[aggropscnt]) {
      if (i != 0) {
        os << " | ";
      }
      else{
    ...
    AggrOp aggregation_op = aggregation_ops[aggropscnt];
...
//201250227
void print_query_fields(std::ostream& os, SelectStmt* select_stmt, int index) {
    auto query_fields = select_stmt->query_fields();
    if (select_stmt->tables().size() != 1) {
        os << query_fields[index].table_name() << ".";
    }
    os << query_fields[index].field_name();
}
void print_aggregation_alias(std::ostream& os, SelectStmt* select_stmt, int index) {
    auto aggregation_alias = select_stmt->aggregation_alias();
    char* alias = aggregation_alias[index];
    if (alias) {
        os << alias;
    }
}
void print_aggregation_op(std::ostream& os, SelectStmt* select_stmt, int index) {
    auto aggregation_ops = select_stmt->aggregation_ops();
    auto aggr_fields = select_stmt->aggr_fields();

    AggrOp aggregation_op = aggregation_ops[index];
    switch (aggregation_op) {
        case COUNT_OP:
            os << "COUNT(";
            break;
    ...
    size_t n = aggr_fields.size();
    if (select_stmt->tables().size() != 1) {
        os << aggr_fields[n - index - 1].table_name() << ".";
    }

    if (aggr_fields[n - index - 1].meta() == nullptr) {
        os << "*)";
    } else {
        os << aggr_fields[n - index - 1].meta()->name() << ")";
    }
}
//void print_aggregation_header(std::ostream& os, SelectStmt* select_stmt) {
//    auto query_fields = select_stmt->query_fields();
...
//
//    int querycnt = 0, aggropscnt = 0;
//    int sum = aggregation_ops.size() + query_fields.size();
//
//    for (int i = 0; i < sum; ++i) {
//        if (i < aggr_op_index.size() && i == aggr_op_index[aggropscnt]) {
//            print_query_fields(os, select_stmt, querycnt);
//            querycnt++;
//            aggropscnt++;
//        }
//
...
//        }
//
//        if (i < aggregation_alias.size()) {
//            print_aggregation_alias(os, select_stmt, i);
//        } else {
//            print_aggregation_op(os, select_stmt, i - aggregation_alias.size());
//        }
//    }
//
//    os << '\n';
...
void tuple_to_string(std::ostream &os, const Tuple &tuple)
{
  ...
  //201250227
  tuple_loop(os, tuple, cell, RC::SUCCESS, true);
...
void value_to_string(std::ostream &os, Value *values, int length) {
    ...
    for (int i = length - 1; i >= 0; i--) {
        ...
        switch (v.type) {
            case INTS:
                convert_int_to_string(os, v.data);
                break;
            case DATES:
                convert_date_to_string(os, v.data);
                break;
            case FLOATS:
                convert_float_to_string(os, v.data);
                break;
            case CHARS:
                convert_char_to_string(os, v.data);
                break;
            default:
                handle_unsupported_type(v.type);
                break;
        }
    }
}
...
IndexScanOperator *ExecuteStage::try_to_create_index_scan_operator(FilterStmt *filter_stmt)
{
  ...
  const FilterUnit *filter_unit = nullptr;
  for (const FilterUnit * filter_unit : filter_units) {
    ...
    if (filter_unit->comp() == NOT_EQUAL || filter_unit->comp() == IS_NOT_OP || filter_unit->comp() == IS_OP) {
      continue;
    }
  ...
  if (left->type() == ExprType::VALUE && right->type() == ExprType::FIELD) {
    ...
    case IS_NOT_OP :     {comp = IS_NOT_OP;} break;
    case IS_OP :     {comp = IS_OP;} break;
    default: {
...
//201250227
bool is_filter_unit_optimizable(const FilterUnit* filter_unit) {
    if (filter_unit->get_type() != Comparison) {
        // TODO: optimize
        return false;
    }
    ...
    CmpOp comp = filter_unit->comp();
    if (comp == NOT_EQUAL || comp == IS_NOT_OP || comp == IS_OP) {
        return false;
    }
...
bool can_find_by_index(Expression* left, Expression* right) {
    if (left->type() == ExprType::FIELD && right->type() == ExprType::VALUE) {
        return true;
    } else if (left->type() == ExprType::VALUE && right->type() == ExprType::FIELD) {
        std::swap(left, right);
...
Index* find_index_for_field(const Field& field) {
    const Table* table = field.table();
    return table->find_index_by_field(field.field_name());
}
...
RC ExecuteStage::aggregation_select_handler(SelectStmt *select_stmt, std::vector<Value> &values, ProjectOperator &project_oper) {
  ...
  while ((rc = project_oper.next()) == RC::SUCCESS) {
    ...
    for (int i = 0; i < aggregation_ops.size(); ++i) {
      ...
      AggrOp aggregation_op = aggregation_ops[i];
      switch (aggregation_op) {
        case COUNT_OP: {
          // if (cell.attr_type() != AttrType::NULLS) {
  ...
  if (select_stmt->having()) {
    ...
    for (int i = 0; i < having_size; ++i) {
      ...
      for (int j = 0; j < having_values.size(); ++j) {
        ...
        if (having_cmp(vs[i], filter.right_value, filter.cmp_op)) {
...
bool having_cmp(Value v1, Value v2, CmpOp cmp) {
  ...
  if (cmp == CmpOp::GREAT_THAN) {
    return vl1 > vl2;
  } else if (cmp == CmpOp::GREAT_EQUAL) {
    return vl1 >= vl2;
  } else if (cmp == CmpOp::EQUAL_TO) {
    return vl1 == vl2;
  } else if (cmp == CmpOp::LESS_EQUAL) {
    return vl1 <= vl2;
  } else if (cmp == CmpOp::LESS_THAN) {
    return vl1 < vl2;
  } else if (cmp == CmpOp::NOT_EQUAL) {
    return vl1 != vl2;
  }
}
  ...
  if (select_stmt->group()) {
    ...
    if (select_stmt->order()) {
      ...
      if (select_stmt->isvaluefunc_ == 1) {
        ...
        value_to_string(ss, select_stmt->func_value_list1_, select_stmt->attr_num_);
        ss << std::endl;
      } else {
        while ((rc = normal_select_handler(select_stmt, tuple, project_oper)) == RC::SUCCESS) {
          ...
          if (select_stmt->isfunc_ == 0) {
            ...
            for (int a = 0; a < select_stmt->attr_num_; a++) {
              if (select_stmt->func_op_list_[a] == NO_FUNCTION_OP) {
                continue;
              }

              else if (select_stmt->func_value_list1_[a].type != UNDEFINED) {
                auto res_tuple = dynamic_cast<ProjectTuple*>(tuple);
                if (res_tuple == nullptr) return RC::MISS_TYPE;
                rc =  res_tuple->modify_cell_spec_at(select_stmt->attr_num_ - a - 1, &select_stmt->func_value_list1_[a], modify, oold);
              } 
                ...
                if (select_stmt->func_op_list_[a] == LENGTH_OP) {
                  ...
                  *(int*)v->data = strlen((char*)cell.data_);
                  ...
                  if (cell.attr_type() != FLOATS || select_stmt->func_value_list2_[a].type != INTS && select_stmt->func_value_list2_[a].type != UNDEFINED) {
                    printf("panic2\n");
                    return RC::MISS_TYPE;
                  }
                  v->type = FLOATS;
                  if (select_stmt->func_value_list2_[a].type == INTS) {
                    *(float*)v->data = round(*(float*)cell.data_, *(int*)(select_stmt->func_value_list2_[a].data));
                  } else {
                  ...
                  v->type = CHARS;
                  v->data = date_format(*(int*)cell.data_, (char*)select_stmt->func_value_list2_[a].data);
                } else {
                  printf("panic4\n");
                  return RC::MISS_TYPE;
                }
...
//201250227
std::vector<const char*> extract_attribute_names(const CreateIndex& create_index) {
    std::vector<const char*> attribute_names;
    for (int i = 0; i < create_index.attribute_num; ++i) {
        attribute_names.push_back(create_index.attribute_name[i]);
    }
...
RC create_index_on_table(Table* table, const char* index_name, const std::vector<const char*>& attribute_names, bool unique) {
    RC rc = table->create_index(nullptr, index_name, attribute_names, unique);
    return rc;
}
void set_create_index_response(SessionEvent* session_event, RC rc) {
    const char *const response = (rc == RC::SUCCESS) ? "SUCCESS\n" : "FAILURE\n";
    session_event->set_response(response);
}
