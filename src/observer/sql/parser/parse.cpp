...
// void condition_init(Condition *condition, FilterType condition_type, CmpOp comp, int left_is_attr, RelAttr *left_attr, Value *left_value,
...
void condition_init_func(Condition *condition, FilterType condition_type, CmpOp comp,
...

{
  ...
  if (left_func_attr != NULL && right_func_attr != NULL) {
    ...
    condition->left_func_op = left_func_attr->func_op;
    ...
    condition->right_func_op = right_func_attr->func_op;
...
{
  //******************************************func******************************************************
  selects->func_op_list[selects->attr_num] = NO_FUNCTION_OP;
  selects->func_value_list1[selects->attr_num].type = UNDEFINED;
  selects->func_value_list2[selects->attr_num].type = UNDEFINED;
  //******************************************func******************************************************
...
{
  selects->func_op_list[selects->attr_num - 1] = function_op;
  selects->is_func = 1;
}
...
void selects_append_first_func_value(Selects *selects, Value * value)
{
  ...
  if (value == 0) selects->func_value_list1[selects->attr_num - 1].type = UNDEFINED;
  else selects->func_value_list1[selects->attr_num - 1] = *value;
}

void selects_append_second_func_value(Selects *selects, Value * value)
{
  if (value == 0) selects->func_value_list2[selects->attr_num - 1].type = UNDEFINED;
  else selects->func_value_list2[selects->attr_num - 1] = *value;
}
...
{
  selects->aggr_op_index[selects->aggregation_num] = selects->aggregation_num + selects->attr_num;
