...
public:
  ...
  const std::vector<AggrOp> &aggregation_ops() const { return aggregation_ops_; }
  const std::vector<char *> &aggregation_alias() const { return aggregation_alias_; }
  const std::vector<AggrOp> &having_ops() const { return having_ops_; }
  ...
  const std::vector<HavingFilter> having_filters() const { return having_filters_; }
...
private:
  ...
  std::vector<AggrOp> aggregation_ops_;
  ...
  std::vector<_HavingFilter> having_filters_;
  std::vector<Field> having_fields_;
  std::vector<AggrOp> having_ops_;
  bool order_;
  bool group_;
  bool having_;
...
//*******************************************func****************************************************
  public:
  FuncOp func_op_list_[MAX_NUM];
  Value func_value_list1_[MAX_NUM];
  Value func_value_list2_[MAX_NUM];
  size_t attr_num_;
  int isfunc_;
  int isvaluefunc_;
  //*******************************************func****************************************************
